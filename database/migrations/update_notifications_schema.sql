...
-- Date: 2025-09-16
-- Description: Update notifications table to support priority escalation, RBAC, and persistent notifications

-- Add new columns to notifications table
ALTER TABLE `notifications`
ADD COLUMN `user_type` ENUM('customer', 'controller', 'controller_nodal', 'admin', 'superadmin') DEFAULT NULL AFTER `customer_id`,
ADD COLUMN `priority` ENUM('low', 'medium', 'high', 'urgent', 'critical') DEFAULT 'medium' AFTER `type`,
ADD COLUMN `related_id` VARCHAR(50) DEFAULT NULL AFTER `complaint_id`,
ADD COLUMN `related_type` VARCHAR(50) DEFAULT NULL AFTER `related_id`,
ADD COLUMN `expires_at` TIMESTAMP NULL DEFAULT NULL AFTER `read_at`,
ADD COLUMN `metadata` JSON DEFAULT NULL AFTER `expires_at`,
ADD COLUMN `dismissed_at` TIMESTAMP NULL DEFAULT NULL AFTER `metadata`,
ADD COLUMN `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER `dismissed_at`;

-- Update existing type enum to include new notification types
ALTER TABLE `notifications`
...
CREATE INDEX `idx_notifications_user_type` ON `notifications` (`user_id`, `user_type`, `is_read`);
...
-- Create notification_settings table for user-specific notification preferences
...
-- Create notification_templates table for managing notification templates
...
INSERT INTO `notification_templates` (`template_code`, `name`, `description`, `subject`, `body_html`, `body_text`, `variables`) VALUES
...
-- Update complaints table to add escalation tracking if not exists
ALTER TABLE `complaints`
ADD COLUMN IF NOT EXISTS `escalated_at` TIMESTAMP NULL DEFAULT NULL AFTER `updated_at`,
ADD COLUMN IF NOT EXISTS `escalation_stopped` TINYINT(1) DEFAULT 0 AFTER `escalated_at`;

-- Add index for escalation tracking
CREATE INDEX IF NOT EXISTS `idx_complaints_escalation` ON `complaints` (`escalated_at`, `escalation_stopped`, `priority`);

-- Create notification_logs table for audit trail (without foreign key initially)
...
-- Add foreign key constraint after table creation (if notifications table has proper primary key)
-- ALTER TABLE `notification_logs` ADD CONSTRAINT `fk_notification_logs_notification_id`
